...
Jahr = 2024
jahr = 24
Modellvariante = f'Basisjahr_{Jahr}_'
...
from x_strombedarf_org import strom_last_24, strom_last_24_original
print("✓ strom_last_24 importiert")

# from x_emobbedarf import e_mob_bedarf
# print("✓ e_mob_bedarf importiert")

e_mob_bedarf = pd.read_excel(rf'data\a_Eingangsdaten\Mobilität\EMob_Zeitreihe_15min_{Jahr}.xlsx')
...
raumwärmebedarf = pd.read_excel(rf'data\a_Eingangsdaten\Wärme\Grundmodell_Raumwärme_Strombedarf_15min_{Jahr}.xlsx')
print("✓ raumwärmebedarf importiert")
warmwasserbedarf = pd.read_excel(rf'data\a_Eingangsdaten\Wärme\Warmwasser_Strombedarf_15min_{Jahr}.xlsx')
print("✓ warmwasserbedarf importiert")
prozesswärmebedarf = pd.read_excel(rf'data\a_Eingangsdaten\Wärme\Prozesswärmebedarf_15min_{Jahr}_weighted.xlsx')
print("✓ prozesswärmebedarf importiert")
#Gesamtbedarf berechnen

##% Strom
#Kopie von des Strombedarfes von {Jahr} erstellen (458TWh - Biomasse und Müll da kein Erweiterungspotential)
bedarf_gesamt = strom_last_24.copy()
...
#Zu strom_last_{jahr} noch eine Spalte mit dem gleichverteilenten zusätzlichen Jahresstrombedarf der Industrie und mech.Energie GHD hinzufügen
jahresstrombedarf_industrie = (400000-185900)+16500  # in GWh: Prognose - Bedarf {Jahr} + mech.Energie GHD (0,5 *32,2TWh durch elektrifizierung)
...
bedarf_gesamt[f'Strom_orginal_{jahr} [MW]'] = strom_last_24_original['Last']
bedarf_gesamt.to_excel(f'data/b_Optimierung/{Modellvariante}Bedarfe_Gesamt_alles_Strom_15min_{Jahr}.xlsx', index=True)

   
#summen check

print('Summencheck der einzelnen Bedarfe in TWh:')
print(f"Strombedarf Original -Biomasse und Müll {Jahr}: {bedarf_gesamt['Strom_Last [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"E-Mobilitätsbedarf {Jahr}: {bedarf_gesamt['E_Mob_Bedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Raumwärmebedarf {Jahr}: {bedarf_gesamt['Raumwärmebedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Warmwasserbedarf {Jahr}: {bedarf_gesamt['Warmwasserbedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Industrie Strombedarf {Jahr}: {bedarf_gesamt['Industrie_Strom_Last [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Wasserstoffbedarf  {Jahr}: {bedarf_gesamt['Wasserstoffbedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Prozesswärmebedarf {Jahr}: {bedarf_gesamt['Prozesswärmebedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"E-Fuel Bedarf Flugzeuge {Jahr}: {bedarf_gesamt['E_Fuel_Flugzeuge [MW]'].sum() / (1000 * 4):.2f} GWh")
print(f"Gesamtbedarf {Jahr}: {bedarf_gesamt['Strom_Gesamt_Bedarf [MW]'].sum() / (1000 * 4):.2f} GWh")
...
summencheck_data = {
    'Kategorie': ['Strombedarf Original {Jahr}- Biomasse und Müll', 'E-Mobilitätsbedarf {Jahr}', 'Raumwärmebedarf {Jahr}', 
                  'Warmwasserbedarf {Jahr}', 'Industrie Strombedarf {Jahr}', 'Wasserstoffbedarf {Jahr}', 
                  'Prozesswärmebedarf {Jahr}', 'E-Fuel Bedarf Flugzeuge {Jahr}', 'Gesamtbedarf {Jahr}',
...
df_summencheck.to_excel( f'data/b_Optimierung/{Modellvariante}summencheck_Bedarfe_Gesamt_alles_Strom_15min_{Jahr}.xlsx',sheet_name='Summencheck', index=False)
