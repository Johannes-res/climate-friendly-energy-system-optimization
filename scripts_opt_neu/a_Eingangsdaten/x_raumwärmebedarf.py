...
df=pd.read_excel(r'data\a_Eingangsdaten\Wärme\Raumwärmebedarf_täglich_23.xlsx', index_col=0)

#Raumwärmebedarf mit Strom decken und COP einbauen

def raumwärme_mit_strom_decken(df):
    """
    ...
    """
    df = df.copy()
    cop_wärmepumpe = 0.075*(df['tavg']+15)+2  # COP-Berechnung basierend auf Außentemperatur und einer Funktion, welche den COP-Wert aus den Korrelationsfaktoren mittelt
    df['Last_Raumwärme [GW]'] = df['Raumwärmebedarf [GWh]'] / 24  # Umrechnung in GW (da tägliche Werte)
    df['Strombedarf_Raumwärme [GW]'] = df['Last_Raumwärme [GW]'] / cop_wärmepumpe
    return df
