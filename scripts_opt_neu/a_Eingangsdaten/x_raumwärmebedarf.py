...
def raumwärme_mit_strom_decken(df, df_tvag):
    """
    Berechnet den Strombedarf zur Deckung des Raumwärmebedarfs mit Wärmepumpen.
    cop_wärmepumpe: Coefficient of Performance der Wärmepumpe
    
    Rückgabe: DataFrame mit zusätzlicher Spalte 'Strombedarf_Raumwärme [GW]'
    """
    df = df.copy()
    df_tvag = df_tvag.copy()
    
    # Stelle sicher, dass beide Indizes datetime sind
    df.index = pd.to_datetime(df.index)
    df_tvag.index = pd.to_datetime(df_tvag.index)
    
    # Extrahiere den Tag aus jedem Index-Eintrag
    tage = df.index.normalize()
    
    # Normalisiere auch den tvag-Index für den Lookup
    df_tvag_normalized = df_tvag.copy()
    df_tvag_normalized.index = df_tvag_normalized.index.normalize()
    
    # Hole die entsprechenden tavg-Werte für jeden Tag mit reindex
    tavg_werte = df_tvag_normalized.loc[tage, 'tavg'].values
    
    # COP-Berechnung basierend auf Außentemperatur
    cop_wärmepumpe = 0.075 * (tavg_werte + 15) + 2
    
    df['Strombedarf_Raumwärme [GW]'] = df['Raumwärmebedarf [GW]'] / cop_wärmepumpe
    return df

def expand_daily_to_15min(df_daily, value_col='Raumwärmebedarf [GWh]', smooth_transitions=True, transition_steps=4, add_peaks=True, peak_factor=1.15):
    """
    Erweitert ein tägliches DataFrame zu einer 15-Minuten-Zeitreihe.
    
    WICHTIG: Alle Berechnungen erfolgen mit ENERGIE [GWh], erst am Ende wird in Leistung [GW] umgerechnet!
    ...
    value_col : str
        Spaltenname der zu erweiternden Werte
    ...
    Rückgabe: 
    ---------
    pd.DataFrame mit 15-min Index und Leistung [GW]
    """
    import numpy as np
    ...
    # Falls Input bereits GWh ist: direkt nutzen
    # Falls Input GW ist: mit 24h multiplizieren
    if 'Strombedarf' in value_col or '[GW]' in value_col:
    ...
    # Jedes 15-min-Intervall bekommt 1/96 der Tagesenergie
    ...
    # ✅ SCHRITT 3: Morgen-/Abendspitzen hinzufügen (falls gewünscht)
    if add_peaks:
        series_energie = add_smooth_energy_peaks(series_energie, tägliche_energie, peak_factor)
    
    # ✅ SCHRITT 4: Tagesübergänge glätten (falls gewünscht)
    if smooth_transitions and len(tägliche_energie) > 1:
    ...
    df_15 = leistung_15min.to_frame()
    df_15.columns = [output_col_name]
    
    return df_15


def add_smooth_energy_peaks(series_energie, tägliche_energie, peak_factor=1.10):
    """
    Fügt gleitende Morgen- (7 Uhr) und Abendspitzen (17 Uhr) mit Gaussian-Profil hinzu.
    ...
    for day in day_list:
        ...
        # Abendspitze: Gaussian-Peak zentriert bei 17 Uhr (Index 68)
        evening_center = 68  # 17:00 Uhr = 68 × 15min
        ...
        # Basis: total_energy / 96 = gleichmäßige Verteilung
        # Mit Peak: Basis × peak_profile
...
df_strom.to_excel(r'data\a_Eingangsdaten\Wärme\Raumwärme_Strombedarf_15min_23.xlsx')




print("Raumwärmebedarf (15min) gespeichert.")
...
df_plot_year = df_strom.resample('1D').mean()
...
ax.plot(
    df_plot_year.index,
    df_plot_year['Raumwärmebedarf [GW]'],
    color='#001450',
    linewidth=2,
    marker='o',
    markersize=None,
    alpha=0.95,
    label='Raumwärmebedarf (thermisch)'
)

ax.plot(
    df_plot_year.index,
    df_plot_year['Strombedarf_Raumwärme [GW]'],
...
plt.tight_layout()
plt.savefig(r'data\a_Eingangsdaten\Wärme\Raumwärmebedarf_Jahresansicht_2023.png', dpi=150, bbox_inches='tight')
plt.close(fig)
print("✓ Jahresansicht gespeichert: Raumwärmebedarf_Jahresansicht_2023.png")
...
ax.plot(
    df_week.index,
    df_week['Strombedarf_Raumwärme [GW]'],
    color='#e74c3c',
    linewidth=2,
    alpha=0.95,
    label='Strombedarf (elektrisch, mit WP-COP)'
...
ax.set_title(f'Raumwärmebedarf - Wochenansicht ({week_start.strftime("%d.%m.")} - {week_end.strftime("%d.%m.%Y")})', 
...
plt.tight_layout()
plt.savefig(r'data\a_Eingangsdaten\Wärme\Raumwärmebedarf_Wochenansicht_2023.png', dpi=150, bbox_inches='tight')
plt.close(fig)
print("✓ Wochenansicht gespeichert: Raumwärmebedarf_Wochenansicht_2023.png")
...
ax.plot(
    df_day.index,
    df_day['Strombedarf_Raumwärme [GW]'],
    color='#e74c3c',
    linewidth=2,
    alpha=0.95,
    label='Strombedarf (elektrisch, mit WP-COP)'
...
ax.set_title(f'Raumwärmebedarf - Tagesansicht ({day_start.strftime("%d.%m.%Y")})', 
             fontsize=14, fontweight='bold', color=text_color)
...
plt.tight_layout()
plt.savefig(r'data\a_Eingangsdaten\Wärme\Raumwärmebedarf_Tagesansicht_2023.png', dpi=150, bbox_inches='tight')
plt.close(fig)
print("✓ Tagesansicht gespeichert: Raumwärmebedarf_Tagesansicht_2023.png")
