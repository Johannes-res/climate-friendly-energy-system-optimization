...
from vorbereitung import bedarf_gesamt as df
...
def analyze_data(df, columns, time_period='Jahr', period_data=None):
    ...
    # Spezielle Analysen für Gesamtbedarf
    bedarf_column = 'Strom_Gesamt_mit_Biomasse_und_Müll [MW]'
    if bedarf_column in columns and bedarf_column in period_data.columns:
        data_gw = pd.to_numeric(period_data[bedarf_column], errors='coerce').dropna() / 1000
        
        # Berechne Lastfaktor (Durchschnittslast / Maximallast)
        load_factor = (data_gw.mean() / data_gw.max()) * 100
        
        # Berechne Spitzenlast-Stunden (> 90% der Maximallast)
        peak_threshold = data_gw.max() * 0.9
        peak_hours = (data_gw > peak_threshold).sum()
        
        # Berechne Grundlast (minimale Last über längeren Zeitraum)
        rolling_min = data_gw.rolling(window=24, min_periods=1).min().min()
        
        print(f"{'='*80}")
        print(f"ZUSÄTZLICHE KENNZAHLEN FÜR GESAMTBEDARF:")
        print(f"{'='*80}\n")
        print(f"  Lastfaktor:              {load_factor:>6.2f} %")
        print(f"  Spitzenlast-Stunden:     {peak_hours:>6.0f} h  (> {peak_threshold:.2f} GW)")
        print(f"  Grundlast (24h-Min):     {rolling_min:>6.2f} GW")
        print()
        
        analysis_results['summary'] = {
            'load_factor': load_factor,
            'peak_hours': peak_hours,
            'base_load': rolling_min
        }
...
selected_columns =      ['Strom_Gesamt_mit_Biomasse_und_Müll [MW]', 'Industrie_Strom_Last [MW]', 'Wasserstoffbedarf [MW]','Prozesswärmebedarf [MW]','E_Fuel_Flugzeuge [MW]', 'Raumwärmebedarf [MW]','Warmwasserbedarf [MW]', 'Strom_orginal_23 [MW]', 'E_Mob_Bedarf [MW]']

custom_labels = ['Gesamtbedarf', 'Industriestrombedarf', 'Wasserstoffbedarf', 'Prozesswärmebedarf', 'Flugtreibstoffbedarf', 'Raumwärmebedarf', 'Warmwasserbedarf', 'Strombedarf orginal', 'E-Mobilitätsbedarf' ]

title =                 'modellierter Strombedarf für das Jahr 2023'
...
analysis_year = analyze_data(df, selected_columns, time_period='Jahresgang 2023', period_data=df)
...
day_data = df[df.index.date == pd.to_datetime(selected_days).date()]
analysis_day = analyze_data(df, selected_columns, time_period=f'Tag {selected_days}', period_data=day_data)
...
title =                                 f'modellierter Strombedarf für die Woche ab {week_start}'
ylabel =                                'Leistung in GW'
...
analysis_week = analyze_data(df, selected_columns, time_period=f'Woche ab {week_start}', period_data=week_data)
...
#selected_columns =                       ['EMobilität', 'Wärmepumpen']
#custom_labels =                         ['Netzlast', 'Modellierung']
title =                                 f'modellierter Strombedarf für den Monat {pd.to_datetime(month_start).strftime("%B %Y")}'
ylabel =                                'Leistung in GW'
...
analysis_month = analyze_data(df, selected_columns, 
                              time_period=f'Monat {pd.to_datetime(month_start).strftime("%B %Y")}', 
