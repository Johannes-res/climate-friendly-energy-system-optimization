...
def main():
    try:
        ...
        print("✓ Stromspeicher-Constraints definiert")
        
        model = c_storage_constraints.wasserstoffspeicher_regeln(model)
        print("✓ H2-Speicher-Constraints definiert")
        
        model = c_constraints.define_constraints(model, a_inputs.df_bedarf, a_inputs.df_bedarf_daily)
        print("✓ Alle Constraints definiert")
        ...
        import traceback
        print("\n❌ FEHLER beim Erstellen des Modells:")
        print("=" * 60)
        traceback.print_exc()
        print("=" * 60)
        raise
